...
target = "flChurn"
to_remove = ["qtdeRecencia", target] + var_identity
...
X_train, X_test, y_train, y_test = model_selection.train_test_split(
    df_train[features], df_train[target], train_size=0.8, random_state=42
)
...
imputer_minus_100 = imputation.ArbitraryNumberImputer(
    arbitrary_number=-100, variables=missing_minus_100
)
...
model = lightgbm.LGBMClassifier(
    n_jobs=-1, learning_rate=0.1, min_child_samples=30, max_depth=10, n_estimators=400
)
...
auc_oot = metrics.roc_auc_score(
    df_oot[target], model_pipeline.predict_proba(df_oot[features])[:, 1]
)
...
model_dict = {"model": model_pipeline, "features": X_train.columns.tolist()}
